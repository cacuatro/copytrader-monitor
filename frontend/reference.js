...
renderCards=function(){referenceCards();document.querySelectorAll('#cards .sc').forEach((card,i)=>{const a=allAccounts[i];if(a.error)return;const top=card.querySelector('.sc-top');top.insertAdjacentHTML('afterbegin',`<span class="strategy-icon">${uiIcon(/gold/i.test(a.name)?'trend':'chart')}</span>`);const stats=card.querySelector('.sc-stats');stats.innerHTML=`<div><div class="ss-l">Saldo</div><div class="ss-v n">${currencyMode==='brl'?fmtBrl(a.balance_brl):fmtBalance(a.balance)}</div></div><div><div class="ss-l">Resultado em 30 dias</div><div class="ss-v ${col(a.profit_month)}">${fmtCurrency(a.profit_month,a.profit_month_brl)}</div></div>`;card.insertAdjacentHTML('beforeend','<span class="strategy-link">Ver detalhes →</span>');card.tabIndex=0;card.setAttribute('role','button');card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();card.click()}})})};
function referenceLayout(){
 document.body.classList.add('reference-layout');
 document.querySelectorAll('#app header .logo,#admin header .logo').forEach(el=>el.innerHTML='<div class="wordmark"><span>K4</span> Trader</div>');
 ...
 const sidebar=document.querySelector('.admin-sidebar');sidebar.insertAdjacentHTML('afterbegin','<div class="wordmark"><span>K4</span> Trader</div>');['home','users','refresh','notice','audit'].forEach((name,i)=>sidebar.querySelectorAll('a')[i].insertAdjacentHTML('afterbegin',uiIcon(name)));
 ...
 document.querySelectorAll('.primary-chart .tabs .tab').forEach(el=>el.textContent=el.textContent.replace('d',' dias'));
}
referenceLayout();
